    ...
    public List<AttendancelogsCount> getLogsCount(Teacher teacher) {
        ...
        courses.forEach(courseVo->{
            ...
            batchCodes.forEach(batchCode->{
                ...
                double absenceStudent = logList.stream().filter(log -> AttendanceTypeEnum.ABSENCE.name.equals(log.getType())).count();
                ...
                count.setAbsenceRate(absenceStudent/logList.size());
            ...
            double absenceRate = countList1.stream().mapToDouble(AttendancelogsCount::getAbsenceRate).sum();
            ...
            count.setAbsenceRate(Double.valueOf(df.format(absenceRate/countList1.size())));
